/**
 * Vendored IBM Carbon glyphs.
 *
 * Iconography rule (abdeen-brand/system/README.md → Iconography): icons are
 * vendored IBM Carbon only — no hand-drawn SVG, no icon fonts, no CDN icon
 * scripts. The brand repo deliberately does not vendor icons; each consuming
 * project vendors the specific glyphs it uses, which is what this file is.
 *
 * Path data is copied verbatim from @carbon/icons 11.85.0 (Apache-2.0,
 * © IBM Corp. 2016, 2023). Every Carbon size of these glyphs draws on the
 * same 32 grid, so one path set serves every render size. Do not edit the
 * path data — re-vendor from the package instead.
 *
 * Icons take `currentColor` and carry no colour of their own: --ink-structure
 * when inactive, --ink-primary when active, and never lacquer unless the icon
 * *is* the signal. Reach for a mono label before reaching for one of these.
 */

interface IconProps {
  /** Rendered size in px. Omit to inherit the box (CSS sets width). */
  size?: number;
  className?: string;
}

function svgProps({ size, className }: IconProps) {
  return {
    className,
    viewBox: "0 0 32 32",
    fill: "currentColor",
...
/** Carbon `fingerprint-recognition` — Touch ID. */
export function FingerprintRecognition(props: IconProps) {
  return (
    <svg {...svgProps(props)}>
