 ...
 * even-odd keyhole path. The 28px plate sits on a 32px canvas so browser tab
 * masks have 2px of breathing room on every edge.
 */
...
const FIELD = `M${INNER_CUT} ${LINE}H${PLATE - LINE}V${PLATE - LINE}H${LINE}V${INNER_CUT}Z`;
// Accent hairline as a true ring (even-odd keyhole); paint after the field.
const RING = `M${CUT} 0H${PLATE}V${PLATE}H0V${CUT}Z${FIELD}`;

const [MX0, MY0, MX1, MY1] = MARK.bbox;
const MARK_SCALE = (PLATE * 0.36) / (MY1 - MY0);
const MARK_TX = PLATE / 2 - MARK_SCALE * ((MX0 + MX1) / 2);
const MARK_TY = PLATE / 2 - MARK_SCALE * ((MY0 + MY1) / 2);
